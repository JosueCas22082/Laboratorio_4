 ...
 * Programacion de microcontadoradorroladores. 
 * Laboratorio4.c
 *
...
int main(void)
{
	...
	while (1)
	{
		//FUNCION DE BOTON
		buton();
		//COMPARACION ENTRE contadorADOR Y ADC
		if (contador == ADCH){
...
void buton(void){ 
	if ((PINC & (1 << PINC1)) == 0){
		_delay_ms(200);					//antirebote
		if ((PINC & (1 << PINC1)) == 2){
			contador = contador + 1;						//aumentar un valor	
			}else{contador = contador;}
		}
		if ((PINC & (1 << PINC2)) == 0)	{
			_delay_ms(200);					//Antirebote
			if ((PINC & (1 << PINC2)) == 4){
				contador = contador - 1 ;						//Disminuir un valor
				}else{contador = contador;}
			}
			PORTB = contador;
		}
